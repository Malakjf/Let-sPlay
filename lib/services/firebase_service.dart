...
class FirebaseService {
  ...
  Future<void> joinMatchTransaction({
    ...
    await _db.runTransaction((transaction) async {
      ...
      // Prevent joining before registry opens
      final openRegistryRaw = data['openRegistryDate'];
      if (openRegistryRaw != null) {
        DateTime registryOpen;
        if (openRegistryRaw is Timestamp) {
          registryOpen = openRegistryRaw.toDate();
        } else if (openRegistryRaw is DateTime) {
          registryOpen = openRegistryRaw;
        } else if (openRegistryRaw is String) {
          registryOpen =
              DateTime.tryParse(openRegistryRaw) ??
              DateTime.fromMillisecondsSinceEpoch(0);
        } else {
          registryOpen = DateTime.fromMillisecondsSinceEpoch(0);
        }
        ...
        if (DateTime.now().isBefore(registryOpen)) {
          ...
          final role = (userDoc.exists ? (userDoc.data()?['role'] ?? '') : '')
              .toString()
              .toLowerCase();
          final canBypass = ['admin', 'coach', 'organizer'].contains(role);
          if (!canBypass) throw Exception('REGISTRY_NOT_OPEN');
        }
      }
      ...
      if (matchDateRaw != null) {
        ...
        } else if (matchDateRaw is DateTime) {
          matchDate = matchDateRaw;
        } else if (matchDateRaw is String) {
          matchDate =
              DateTime.tryParse(matchDateRaw) ??
              DateTime.fromMillisecondsSinceEpoch(0);
        } else {
          matchDate = DateTime.fromMillisecondsSinceEpoch(0);
        }
